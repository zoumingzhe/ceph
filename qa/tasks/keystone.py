...
def create_endpoint(ctx, cclient, service, url):
    ...
    return run_section_cmds(ctx, cclient, 'endpoint create {}'.format(service), 'url',
                            [endpoint_section])
...
def fill_keystone(ctx, config):
    ...
    for (cclient, cconfig) in config.items():
        ...
        admin_url = 'http://{host}:{port}/v3'.format(host=public_host,
                                               port=admin_port)
        create_endpoint(ctx, cclient, 'identity admin', admin_url)
        create_endpoint(ctx, cclient, 'identity internal', url)
        create_endpoint(ctx, cclient, 'identity public', url)
